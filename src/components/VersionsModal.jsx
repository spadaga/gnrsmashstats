...
export default function VersionsModal({ open, onClose, onRestored }) {
  ...
  return createPortal(
    <>
        ...
        <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md p-6">
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">3 daily snapshots — restore any day's state</p>
            </div>
          </div>
          {loading ? (
            <div className="py-10 text-center text-slate-400 text-sm">Loading…</div>
          ) : versions.length === 0 ? (
            <div className="py-10 text-center text-slate-400 dark:text-slate-500 text-sm">No snapshots yet. Snapshots are created automatically on the first write each day.</div>
          ) : (
            <div className="space-y-2">
              {versions.map((v) => (
                <div key={v.ts} className="flex items-center justify-between px-4 py-3 rounded-xl border dark:border-slate-700 hover:border-orange-300 hover:bg-orange-50 dark:hover:bg-orange-900/20 dark:hover:border-orange-800 transition">
                  <div>
                    <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">{formatTs(v.ts)}</p>
                    <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                      {v.matchCount ?? '—'} matches · {v.playerCount ?? '—'} players{v.slotCount != null ? ` · ${v.slotCount} slots` : ''}
                    </p>
                  </div>
                  <button onClick={() => setConfirm(v.ts)}
      ...
      <ConfirmDialog open={!!confirm} title="Restore this snapshot?"
