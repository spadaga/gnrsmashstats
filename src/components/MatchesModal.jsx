...
function groupByDate(matches) {
  const map = {}
  sortMatchesDesc(matches).forEach((m) => { (map[m.date] ||= []).push(m) })
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg mt-8 p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-bold uppercase tracking-wide text-slate-700 dark:text-slate-300">
            {title} <span className="text-slate-400 dark:text-slate-500 font-medium normal-case">({matches.length})</span>
          </h3>
            ...
            {groups.map(({ date, items }) => (
              <div key={date}>
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="text-[11px] font-bold uppercase tracking-wide text-slate-400 dark:text-slate-500">{date}</span>
                  <div className="flex-1 h-px bg-slate-100 dark:bg-slate-700" />
                  <span className="text-[10px] font-semibold text-slate-500 dark:text-slate-400">{items.length} match{items.length !== 1 ? 'es' : ''}</span>
                </div>
