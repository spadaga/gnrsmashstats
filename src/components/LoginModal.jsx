...
export default function LoginModal({ open, players, onLogin, onClose }) {
  ...
  function handlePinInput(e) {
    ...
    if (val.length === 4) {
      ...
      if (admin) {
        setFound(admin);
        setTimeout(() => onLogin(admin.name), 700);
    ...
    >
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-xs p-8">
        ...
        <div className="flex flex-col items-center text-center">
          <span
            className={`w-14 h-14 rounded-full flex items-center justify-center mb-4 transition-colors duration-300 ${found ? "bg-emerald-100 dark:bg-emerald-900/30" : "bg-orange-100 dark:bg-orange-900/30"}`}
          >
            {found ? (
            ...
            <>
              ...
              <div className="flex gap-3 mb-5">
                {[0, 1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className={`w-4 h-4 rounded-full border-2 transition-all duration-150 ${pin.length > i ? "bg-orange-600 border-orange-600 scale-110" : "bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-600"}`}
                  />
                ))}
              </div>
              <input
