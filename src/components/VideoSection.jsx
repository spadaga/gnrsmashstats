...
export default function VideoSection({ videos, onAdd, onDelete, isAdmin }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl border dark:border-slate-700 p-4">
      ...
      {isAdmin && editing ? (
        <div className="space-y-3">
          ...
          <div className="space-y-1 max-h-64 overflow-y-auto">
            {videos.map((v, i) => (
              <div key={i} className="flex items-center justify-between px-2 py-1.5 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 text-sm">
                <span className="truncate flex-1 text-slate-600 dark:text-slate-300">{v}</span>
                <button onClick={() => setConfirm(i)} className="text-slate-300 hover:text-red-500 shrink-0 ml-2"><Trash2 size={14} /></button>
              </div>
            ))}
      ...
      <ConfirmDialog open={confirm !== null} title="Remove this video?" message="The video link will be removed."
        confirmLabel="Remove" onConfirm={() => { onDelete(confirm); setConfirm(null) }} onCancel={() => setConfirm(null)} />
    </div>
  )
}