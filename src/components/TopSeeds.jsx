      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg mt-8 p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-bold uppercase tracking-wide text-slate-700 dark:text-slate-300 flex items-center gap-2">
            <Trophy size={15} className="text-orange-600" /> All Pair
            Combinations
          </h3>
          <button
            onClick={onClose}
            ...
            {pairs.map((p, i) => (
              ...
              >
                <div className="flex items-center gap-2.5">
                  <span
                    className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold shrink-0 ${i === 0 ? "bg-orange-600 text-white" : "bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400"}`}
                  >
                    {i + 1}
                  </span>
                  <div>
                ...
                <span
                  className={`text-sm font-bold ${i === 0 ? "text-orange-600" : "text-slate-700 dark:text-slate-300"}`}
                >
                  {p.winRate}%
                </span>
              </div>
...
export default function TopSeeds({ matches }) {
  ...
  const pairs = computeTopPairs(filtered, minMatches);
  const top2 = pairs.filter((p) => p.qualified).slice(0, 2);
    ...
    <div>
      ...
      {top2.length === 0 && (
        <p className="text-slate-400 text-sm text-center py-6 bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-2xl">
          No qualified pairs {periodLabel === "overall" ? "yet" : periodLabel}{" "}
          yet.
        </p>
      )}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {top2.map((p, i) => (
          <button
            ...
            onClick={() =>
              setDrilldown({
                title: `${p.players.join(" & ")} matches`,
                list: matchesForPair(filtered, p.players),
              })
            }
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <span
                className={`text-[11px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full ${i === 0 ? "bg-white/20" : "bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400"}`}
              >
                Top Seed #{i + 1}
              </span>
              <span className="text-right">
                ...
                <span
                  className={`text-[10px] uppercase tracking-wide ${i === 0 ? "text-orange-100" : "text-slate-400 dark:text-slate-500"}`}
                >
                  Win Rate
                </span>
              </span>
            ...
            <Trophy
              size={56}
              className={`absolute -bottom-2 -right-2 ${i === 0 ? "text-white/10" : "text-slate-100 dark:text-slate-700"}`}
            />
            <p
              className={`font-bold text-sm relative leading-tight ${i !== 0 ? "text-slate-900 dark:text-white" : ""}`}
            >
              {p.players.join(" & ")}
            </p>
            <p
              className={`text-xs relative mt-0.5 ${i === 0 ? "text-orange-100" : "text-slate-500 dark:text-slate-400"}`}
            >
              {p.wins}W – {p.losses}L
            </p>
          </button>
      ...
      {showAll && (
        <PairAllModal matches={matches} onClose={() => setShowAll(false)} />
      )}
      {drilldown && (
        <MatchesModal
