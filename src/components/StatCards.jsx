    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center p-4 overflow-y-auto" onClick={(e) => e.target === e.currentTarget && onClose()}>
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm mt-8 p-5">
        ...
        <div className="space-y-1 max-h-[60vh] overflow-y-auto pr-1">
          {players.map((name) => (
            <div key={name} className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700">
...
export default function StatCards({ matches, players }) {
  ...
  return (
    <div className="rounded-2xl bg-orange-600 text-white p-5 flex items-center justify-between gap-4 flex-wrap">
      <button type="button" onClick={() => setShowMatches(true)} className="flex items-center gap-3 text-left hover:opacity-90 transition">
      ...
      <button type="button" onClick={() => setShowPlayers(true)} className="flex items-center gap-3 text-left hover:opacity-90 transition">
      ...
      {showMatches && <MatchesModal title="Matches in this period" matches={matches} onClose={() => setShowMatches(false)} />}
      {showPlayers && <PlayersModal players={names} onClose={() => setShowPlayers(false)} />}
    </div>
  )
}
