...
export default async function handler(req, res) {
  ...
  const param =
    parts[2] !== undefined ? decodeURIComponent(parts[2]) : undefined;

  try {
    const state = await readState();

    if (resource === "versions" && req.method === "GET") {
      const versions = (await listSnapshots())
    ...
    if (resource === "export" && req.method === "GET") {
      res.setHeader(
        "Content-Disposition",
        'attachment; filename="badminton-results.json"',
      );
      return res.status(200).json(state);
    }
    ...
    if (resource === "import" && req.method === "POST") {
      ...
      if (Array.isArray(body.videos))
        next.videos = body.videos.slice(0, MAX_VIDEOS);
    ...
    if (resource === "players") {
      ...
      if (req.method === "PUT") {
        ...
        if (inactive !== undefined) {
          updated.inactive = Boolean(inactive);
        } else if (existing.inactive) {
          updated.inactive = existing.inactive;
        }
    ...
    if (resource === "videos") {
      if (req.method === "POST") {
        ...
        if (state.videos.length >= MAX_VIDEOS)
          return res
            .status(400)
            .json({ error: `Max ${MAX_VIDEOS} videos reached` });
        await snapshotState(state);
    ...
    if (resource === "slots") {
      if (req.method === "POST") {
        ...
        return res
          .status(200)
          .json(await addSlot({ ...slot, id: crypto.randomUUID() }));
      }
      if (req.method === "PUT") {
        return res
          .status(200)
          .json(await updateSlotById(param, req.body || {}));
      }
      if (req.method === "DELETE") {
        await snapshotState(state);
    ...
    if (resource === "dues") {
      if (req.method === "POST") {
        ...
        return res
          .status(200)
          .json(await addDue({ ...due, id: crypto.randomUUID() }));
      }
      if (req.method === "PUT") {
    ...
    if (resource === "photos") {
      if (req.method === "POST") {
        ...
        if (state.photos.length >= MAX_PHOTOS)
          return res
            .status(400)
            .json({ error: `Max ${MAX_PHOTOS} photos reached` });
        await snapshotState(state);
        return res
          .status(200)
          .json(await addPhoto(await savePhotoBlob(dataUrl)));
      }
      if (req.method === "DELETE") {
